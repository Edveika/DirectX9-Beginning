...
LPDIRECT3DVERTEXBUFFER9 CreateVertexBuffer(int vertexCount);
void* LockVertexBuffer(LPDIRECT3DVERTEXBUFFER9 buffer);
void LoadDataToLockedVertexBuffer(void* &pVertices);
...
bool InitWindow(HINSTANCE hInstance)
{
	...
	wcex.lpszMenuName = L"DirectX9VertexBuffers";    // name of menu resource 
	wcex.lpszClassName = L"DirectX9VertexBuffers";  // name of window class 
	...
	(
		L"DirectX9VertexBuffers", // window class to use
		L"DirectX9VertexBuffers", // window title
...
LPDIRECT3DVERTEXBUFFER9 CreateVertexBuffer(int vertexCount)
{
	...
	// variable used hold to return code
	HRESULT hr;

	hr = pd3dDevice->CreateVertexBuffer
	(
		vertexCount * sizeof(CUSTOMVERTEX), // size in bytes of the buffer
		0,
		...
		&buffer, // this variable holds newly created buffer
		NULL
	);

	// if result failed or buffer is null
...
void* LockVertexBuffer(LPDIRECT3DVERTEXBUFFER9 buffer)
{
	// Result
	HRESULT hr;
	// Void pointer to buffer that holds the vertices
	void* pVertices;
	...
	// Return void ptr to locked vertices
	return pVertices;
}
...
HRESULT SetupVertexBuffer()
{
	...
	CUSTOMVERTEX vertices[] =
	{
		{10.0f, 50.0f, 0.5f, 1.0f, D3DCOLOR_ARGB(100,0,255,0)},
		{150.0f, 400.0f, 0.5f, 1.0f, D3DCOLOR_ARGB(100,0,255,0)},
		{50.0f, 400.0f, 0.5f, 1.0f, D3DCOLOR_ARGB(100,0,255,0)}
	};

	hr = pd3dDevice->CreateVertexBuffer
	(
		3 * sizeof(CUSTOMVERTEX), // size in bytes of the buffer
		0,
		D3DFVF_XYZRHW | D3DFVF_DIFFUSE, // because vertex is untransformed
		D3DPOOL_DEFAULT, // allocates the buffer in the most appropriate memory for this type
